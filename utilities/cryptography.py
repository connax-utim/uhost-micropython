...
class CryptoLayer(object):
    ...
    def decrypt(self, message):
        ...
        Decrypt message
        """
        logging.info("============len {}".format(len(message)))
        if len(message) < 2:
            return None
        logging.info("============msg {}".format(message[1:2]))
    ...
    def sign(self, mode, message):
        ...
        if self.__key is not None and mode != self.SIGN_MODE_NONE:
            logging.debug('Signing mode {}'.format(mode))
            if mode == self.SIGN_MODE_SHA256:
                signature = hmac.new(self.__key, message, hashlib.sha256).digest()
                return Tag.CRYPTO.SIGNED + mode + message + signature
        logging.debug(Tag.CRYPTO.SIGNED + self.SIGN_MODE_NONE + message)
    ...
    def unsign(self, message):
        ...
        if self.__key is None:
            ...
            ref_signature = hmac.new(self.__key, useful_message, hashlib.sha256).digest()
