...
def gen_x(hash_class, salt, username, password):
    ...
    print('user =', username)
    print('pass =', password)
    print('H =', H(hash_class, (username + b':' + password)))
...
def create_salted_verification_key(username, password, hash_alg=SHA256, ng_type=NG_1024, n_hex=None,
    ...
    x = gen_x(hash_class, _s, username, password)
    print('g = {}\nx = {}'.format(g, x))
    _v = long_to_bytes(pow(g, x, N))
...
class Verifier(object):
    def __init__(self, username, bytes_s, bytes_v, bytes_A, hash_alg=SHA256, ng_type=NG_1024,
        ...
        print('A bytes to long: {}'.format(self.A))
        ...
        if not self.safety_failed:
            ...
            print('HASHMARE', self.A, self.B, self.u, sep='\n')
            ...
            self.M = calculate_M(hash_class, N, g, self.identity, self.s, self.A, self.B, self.K)
            self.H_AMK = calculate_H_AMK(hash_class, self.A, self.M, self.K)
            print('WE HAVE HAMK WTF')
    ...
    def verify_session(self, user_M):
        if not self.safety_failed and user_M == self.M:
            ...
            print('VERIFY', (not self.safety_failed), user_M == self.M)
...
class User(object):
    ...
    def start_authentication(self):
        return (self.identity, long_to_bytes(self.A))
    ...
    def process_challenge(self, bytes_s, bytes_B):
        ...
        self.x = gen_x(hash_class, self.s, self.identity, self.p)
