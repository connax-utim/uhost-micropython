...
class ProcessInboundItem(object):
    ...
    def run_worker(self, tlv_data):
        ...
        logging.debug("Payload before: %s", str(payload))
        ...
        logging.debug("Payload during: %s", str(payload))
        ...
        if payload is not None:
            ...
            if devid in self.__uhost.database.get_utim_names():
                ...
                if tag == Tag.UCOMMAND.HELLO:
                    ...
                    self.__connection_status_worker.process(devid, payload,
                                                            self.__outbound_data_queue,
