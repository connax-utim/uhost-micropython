...
class DataBaseConnection(object):
    ...
    def __execute(self, sql):
        ...
        while tries < self.__MAX_TRIES:
            try:
                connection = mysql.connector.connect(user=self.__db_name,
    ...
    def __create_table(self, db_name):
        ...
        sql = """CREATE TABLE IF NOT EXISTS {db_name} (
            ...
            status_db_name=self.__STATUSES_DB_NAME
        )
        logging.debug(sql)
        cursor.execute(sql)
        ...
        for item in self.__STATUSES_LIST:
            ...
            if len(item) == 4:
                try:
                    ...
                    """.format(table_name=self.__STATUSES_DB_NAME,
                               cstatus=item[0],
                               status=item[1],
                               network=item[2],
                               security=item[3])
    ...
    def does_utim_exist(self, devid):
        ...
        """
        sql = "SELECT device_id FROM {db_name} WHERE device_id = '{devid}'".format(
            db_name=self.__DB_NAME,
            devid=devid)
        fetch = self.__execute(sql)
    ...
    def get_session_key(self, devid):
        ...
        sql = "SELECT session_key FROM {db_name} WHERE device_id = '{devid}'".format(
            db_name=self.__DB_NAME,
            devid=devid)
        fetch = self.__execute(sql)
    ...
    def get_config_hash(self, devid):
        ...
        """
        sql = "SELECT config_hash FROM {db_name} WHERE device_id = '{devid}'".format(
            db_name=self.__DB_NAME,
            devid=devid)
        fetch = self.__execute(sql)
        if fetch is not None and len(fetch) > 0:
    ...
    def get_keep_alive_counter(self, devid):
        ...
        :return int: counter
        """
        sql = "SELECT keep_alive_counter FROM {db_name} WHERE device_id = '{devid}'".format(
            db_name=self.__DB_NAME,
            devid=devid)
        fetch = self.__execute(sql)
        if fetch is not None and len(fetch) > 0:
    ...
    def get_status(self, devid):
        ...
        :return str: status
        """
        sql = "SELECT status FROM {db_name} WHERE device_id = '{devid}'".format(
            db_name=self.__DB_NAME,
            devid=devid)
        fetch = self.__execute(sql)
        if fetch is not None and len(fetch) > 0:
